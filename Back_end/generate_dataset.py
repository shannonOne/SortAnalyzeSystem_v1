    ...
    for seq_len in range(1, Seq_len_max + 1):  # 1~seq_len_max 从长度为1开始生成
        ...
        for n in range(Sample_num):
            ...
            if Sort_type == 'bubble_sort':
                [sorted_seq, compare_count, move_count] = bubble_sort(rand_seq)
            elif Sort_type == 'quick_sort':
                [sorted_seq, compare_count, move_count] = quick_sort(rand_seq)
            elif Sort_type == 'select_sort':
                [sorted_seq, compare_count, move_count] = select_sort(rand_seq)
            elif Sort_type == 'insert_sort':
                sorted_seq, compare_count, move_count = insert_sort(rand_seq)
            elif Sort_type == 'merge_sort':
                sorted_seq, compare_count, move_count = merge_sort(rand_seq)
            elif Sort_type == 'heap_sort':
                sorted_seq, compare_count, move_count = heap_sort(rand_seq)
...
if __name__ == '__main__':
    ...
    min = 0  # 数组元素最小值
    max = 100  # 数组元素最大值
    seq_len_max = 32  # 最大输入规模（数组长度)
    sample_num = 3  # 对于每个规模, 生成样本数量, 总计样本数量= seq_len_max * example_num
    seq_method = '02'

    data_folder = '/Users/xyf/Desktop/AnalyzeSort_System_v2/Back_end/data/'
