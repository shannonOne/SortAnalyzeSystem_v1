...
sort_names = ['选择排序', '冒泡排序', '快速排序', '堆排序', '归并排序', '插入排序'];
curpage = 0;
...
function show_analyze_result(max_n, data_method) {
    ...
    option = create_option(['选择排序', '冒泡排序', '快速排序', '堆排序', '归并排序', '插入排序', 'n^2', 'nlogn'],
    ...
    option = create_option(legend, x_data, [analyze_result_compare[0][3], analyze_result_excahnge[0][3],
        baseline_n2, baseline_nlog
    ], '选择排序的输入规模 n', '基本运算次数');
    choose_chart.clear();
    choose_chart.setOption(option);
    option = create_option(legend, x_data, [analyze_result_compare[1][3], analyze_result_excahnge[1][3],
        baseline_n2, baseline_nlog
    ], '冒泡排序的输入规模 n', '基本运算次数');
    bubble_chart.clear();
    bubble_chart.setOption(option);
    option = create_option(legend, x_data, [analyze_result_compare[2][3], analyze_result_excahnge[2][3],
        baseline_n2, baseline_nlog
    ], '快速排序的输入规模 n', '基本运算次数');
    quick_chart.clear();
    quick_chart.setOption(option);
    option = create_option(legend, x_data, [analyze_result_compare[3][3], analyze_result_excahnge[3][3],
        baseline_n2, baseline_nlog
    ], '堆排序的输入规模 n', '基本运算次数');
    heap_chart.clear();
    heap_chart.setOption(option);
    option = create_option(legend, x_data, [analyze_result_compare[4][3], analyze_result_excahnge[4][3],
        baseline_n2, baseline_nlog
    ], '归并排序的输入规模 n', '基本运算次数');
    merge_chart.clear();
    merge_chart.setOption(option);
    option = create_option(legend, x_data, [analyze_result_compare[5][3], analyze_result_excahnge[5][3],
        baseline_n2, baseline_nlog
    ], '插入排序的输入规模 n', '基本运算次数');
    insert_chart.clear();
